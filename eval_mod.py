...
def load_model(
    ...
    elif init_from == 'gpt2m':
        model = GPT2LMHeadModel.from_pretrained("gpt2-medium")
    elif init_from == 'gpt2l': 
        model = GPT2LMHeadModel.from_pretrained("gpt2-large")
    elif init_from == 'gpt2xl':
...
if __name__ == "__main__":
    ...
    do_scores = get_sentence_scores(test_sentences['DOsentence'].tolist())
    test_sentences['DO_score'] = list(map(lambda x: x[0], do_scores))

    po_scores = get_sentence_scores(test_sentences['PDsentence'].tolist())
    test_sentences['PD_score'] = list(map(lambda x: x[0], po_scores))

    test_sentences['diff'] = test_sentences['PD_score'] - test_sentences['DO_score']

    test_sentences.to_csv(f'wider_pairs_scored_{model_name}.csv')
